...
export class CalendarComponent implements OnInit {
  ...
  async prepareData() {
    ...
    this.allowUserSearch = !this.isArtistView;
    ...
    if (this.isUserSelected) {
      if (this.user && this.user.id) {
        await this.getLeaveInfoByArtist(this.user.id);
      } else {
  ...
  isDayOff(day: any) {
    ...
    if (AppConstants.WEEK_ENDS.includes(weekdayFull)) {
      ...
      //console.log(this.leaveDates, " ** ", dateStr);
      day.label = "Leave";
      return true;
    }
    return false;
  ...
  updateData() {
    this.data.forEach((element) => {
      if (element && element.date) {
        var formatedString = format(new Date(element.date), "dd-LL-yyyy");
        ...
        if (_yearIndex > -1) {
          ...
          if (this.formattedMonthArray[_monthIndex] !== null) {
            this.formattedMonthArray[_monthIndex].date[_day - 1].progress =
              element.progress;
            this.formattedMonthArray[_monthIndex].date[_day - 1].hours =
              element.hours;
          }
          this.log(
  ...
  getText(_day) {
    ...
    if (_day.progress == 0) {
      if (obj && obj.offType && obj.offType.toLowerCase() == "leave") {
        if (obj.overAllPer > 0) {
          return "S-Leave";
        }
      }
